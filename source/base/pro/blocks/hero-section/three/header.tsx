...
export default function Header() {
  ...
  return (
    ...
    >
      <div
        className={cn(
          "border-foregroud/5 absolute inset-x-0 top-0 z-50 transition-[background-color,border-color,box-shadow,backdrop-filter] duration-300",
        ...
        if (link.name && link.href) {
          return (
            <Link
              ...
              href={link.href}
              onClick={closeMenu}
              className="group relative block py-4 text-lg font-medium"
            >
              {link.name}
            </Link>
          )
...
const NavMenu = () => {
  return (
    <NavigationMenu className="[--color-muted:color-mix(in_oklch,var(--color-foreground)_5%,transparent)] [--viewport-outer-px:2rem] [--viewport-top:3.25rem] **:data-[slot=navigation-menu-viewport]:rounded-2xl **:data-[slot=navigation-menu-viewport]:shadow-lg max-lg:hidden">
      <NavigationMenuList className="gap-3">
        <NavigationMenuItem value="product">
          <NavigationMenuTrigger>Product</NavigationMenuTrigger>
          <NavigationMenuContent className="origin-top p-1">
            <div className="grid w-full min-w-270 grid-cols-4 gap-1">
              ...
              <div className="row-span-2 grid grid-rows-subgrid">
                <div
                  data-theme="quartz"
                  className="relative row-span-2 grid overflow-hidden rounded-xl bg-emerald-100 bg-linear-to-b from-white via-white/50 to-sky-100 p-1 inset-ring-1 inset-ring-foreground/10 transition-colors duration-200 hover:bg-emerald-50"
                >
    ...
    <li {...props}>
      <NavigationMenuLink
        render={
          <Link
            href={href}
            className="grid grid-cols-[auto_1fr] gap-2.5 rounded-lg"
          >
