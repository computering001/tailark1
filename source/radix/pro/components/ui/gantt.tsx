      ...
      for (const month of quarter.months) {
        ...
        headers.push(
          <div
            ...
            key={`${year.year}-${monthNumber}`}
          >
            <GanttContentHeader
              title={format(new Date(year.year, monthNumber, 1), "MMMM yyyy")}
              columns={month.days}
              renderHeaderItem={(item: number) => (
                <div className="flex items-center justify-center gap-1">
                  <p>
                    {format(
                      addDays(new Date(year.year, monthNumber, 1), item),
                      "d"
                    )}
                  </p>
            ...
            <GanttColumns
              ...
              isColumnSecondary={(item: number) =>
                [0, 6].includes(
                  addDays(new Date(year.year, monthNumber, 1), item).getDay()
                )
              }
            />
          </div>
...
export const GanttSidebarItem: FC<GanttSidebarItemProps> = ({
  ...
  const duration = tempEndAt
    ...
    : `${formatDistance(feature.startAt, typeof window !== "undefined" ? new Date() : new Date("2024-10-25"))} so far`
...
export const GanttFeatureDragHelper: FC<GanttFeatureDragHelperProps> = ({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "absolute top-1/2 h-[80%] w-1 -translate-y-1/2 rounded-sm bg-muted-foreground opacity-0 transition-opacity",
          direction === "left" ? "left-2.5" : "right-2.5",
...
export const GanttFeatureItemCard: FC<GanttFeatureItemCardProps> = ({
  ...
  return (
    <Card className="h-full w-full rounded-md border-transparent bg-card p-2 text-xs shadow-sm ring-1 ring-foreground/10">
      <div
        className={cn(
...
export const GanttFeatureItem: FC<GanttFeatureItemProps> = ({
  ...
  const previousStartAtRef = useRef(startAt)
  const previousEndAtRef = useRef(endAt)
  ...
  const handleItemDragStart = () => {
    previousMouseXRef.current = mousePosition.x
    previousStartAtRef.current = startAt
    previousEndAtRef.current = endAt
  }

  const handleItemDragMove = () => {
    const currentDate = getDateByMousePosition(gantt, mousePosition.x)
    const originalDate = getDateByMousePosition(gantt, previousMouseXRef.current)
    const delta =
      gantt.range === "daily"
        ? getDifferenceIn(gantt.range)(currentDate, originalDate)
        : getInnerDifferenceIn(gantt.range)(currentDate, originalDate)
    const newStartDate = addDays(previousStartAtRef.current, delta)
    const newEndDate = previousEndAtRef.current
      ? addDays(previousEndAtRef.current, delta)
      : null
    ...
    >
      <ContextMenu>
        <ContextMenuTrigger asChild>
          ...
          >
            {label}
            <span className="max-h-0 overflow-hidden opacity-80 transition-[max-height] group-hover:max-h-[2rem]">
              {formatDate(date, "MMM dd, yyyy")}
            </span>
          </div>
        </ContextMenuTrigger>
...
export const GanttProvider: FC<GanttProviderProps> = ({
  ...
  const [timelineData, setTimelineData] = useState<TimelineData>(() =>
    createInitialTimelineData(
      typeof window !== "undefined" ? new Date() : new Date("2024-10-25")
    )
  )
  ...
  const scrollRefCallback = useCallback((node: HTMLDivElement | null) => {
    if (node) {
      const sidebarElement = node.querySelector(
        '[data-roadmap-ui="gantt-sidebar"]'
      )
  ...
  return (
    <GanttContext.Provider value={contextValue}>
      <div
        className={cn(
          "gantt relative grid h-full w-full flex-none overflow-auto rounded-sm bg-secondary select-none",
          range,
          className
        )}
        style={{
