        ...
        <section className="bg-background py-16 md:py-32">
            <div className="@container mx-auto max-w-5xl px-6">
                <div className="text-center">
                    <h2 className="text-foreground mx-auto max-w-xs text-balance text-4xl font-semibold tracking-tight">Our incredible leadership team</h2>

                    <div className="@max-2xs:grid-cols-1 mt-24 grid grid-cols-2 gap-16 sm:grid-cols-3 lg:grid-cols-4">
                        {members.map((member) => (
                            <div key={member.name}>
                                <div className="shadow-black/3 before:border-foreground/6.5 before:z-1 relative mx-auto aspect-square size-24 overflow-hidden rounded-full shadow-md before:absolute before:inset-0 before:rounded-full before:border">
                                    <div
                                        aria-hidden
                                        className={cn('bg-linear-to-r z-1 pointer-events-none absolute inset-0 size-40 rounded-full opacity-35 mix-blend-overlay blur-2xl will-change-transform md:size-72', member.decoratorColors)}
                                    />
                                    <Image
                                        src={member.image}
                                        alt={member.name}
