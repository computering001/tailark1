                    ...
                    <div className="bg-background relative overflow-hidden rounded-2xl p-3 sm:p-12">
                        <AnimatePresence
                            ...
                            <m.div
                                ...
                                className="bg-card *:scale-85 ring-border shadow-black/4 relative z-10 flex aspect-square h-full items-center justify-center rounded-xl shadow-md ring md:aspect-video"
                                key={expandedIndex}>
                        ...
                        <div className="flex max-lg:-ml-3">
                            {features.map((feature, index) => (
                                <button type="button"
                                    ...
                                    className="active:scale-98 not-data-[expanded=true]:hover:bg-foreground/5 group flex cursor-pointer items-center gap-3 rounded-lg px-3 py-2 text-left transition-[transform,background-color,color,box-shadow] duration-200">
                                    <span className={cn('group-hover:text-foreground text-sm font-medium transition-colors', expandedIndex === index ? 'text-foreground' : 'text-muted-foreground')}>{feature.title}</span>
                                </button>
                            ))}
                        </div>

                        <p className="text-muted-foreground mt-2 max-w-xl text-balance text-lg lg:pl-3">{features[expandedIndex].description}</p>
                    </div>
                </div>
            </div>
        </section>
